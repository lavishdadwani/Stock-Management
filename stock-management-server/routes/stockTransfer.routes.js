...
router.post('/transfer', Auth, authorize('manager', 'owner', 'super_admin'), transferStock);

// Get all stock transfers - only managers and owners (can view all transfers)
router.get('/get-all', Auth, authorize('manager', 'owner', 'super_admin'), getAllStockTransfers);

// Get stock transfer quantities - managers, owners, and core team (core team can only see their own stock)
...
router.put('/update/:id', Auth, authorize('manager', 'owner', 'super_admin'), updateStockTransfer);
